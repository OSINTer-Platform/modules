...
def processTweetData(tweetData):
    ...
    for tweet in tweets:
        ...
        if "entities" in tweet:
            entitySpecs = {
                    "hashtags" : {"identifier" : "tag", "link" : "https://twitter.com/hashtag/", "prepend" : "#"},
                    "mentions" : {"identifier" : "username", "link" : "https://twitter.com/", "prepend" : "@"},
                    }
            ...
            for entityName in entitySpecs:
                if entityName in tweet["entities"]:
                    ...
                    for entity in tweet["entities"].pop(entityName):
                        ...
                        tweet["text"] = tweet["text"].replace(f"{entitySpecs[entityName]['prepend']}{ID}", f"[{entitySpecs[entityName]['prepend']}{ID}]({entitySpecs[entityName]['link']}{ID})")
            ...
            if "urls" in tweet["entities"]:
                for URLDetails in tweet["entities"].pop("urls"):
                    if "title" in URLDetails:
                        ...
                        tweet["text"] = tweet["text"].replace(URLDetails["url"], f"[{URLDetails['display_url']}]({URLDetails['expanded_url']})")
